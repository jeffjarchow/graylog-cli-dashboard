...
// 		.map((message) => {
// 			var parts = message.full_message.split('\n');
// 			parts.join(',');
// 			var arr;
// 			parts.forEach(function(mes) {
// 				// console.log(mes);
// 				// arr.push(`${moment(message.timestamp).format()} - ${mes}`);
// 			})
// 			return [message.full_message.split('\n')];
// //			return `${moment(message.timestamp).format()} - ${parts}`;
// 		});

		var items = [];
		i.forEach(function(line) {
			var m = line.full_message.split('\n');
			m.forEach(function(mes) {
				if (mes.includes('body')) {
					var pieces = mes.split('{');
					items.push(`${moment(line.timestamp).unix()} - ${pieces.shift()} {`);
					var middle = pieces[0].split('}');
					middle.unshift();
					var body = middle[0].split(',');
					body.forEach(function(b) {
						items.push(`${moment(line.timestamp).unix()} - \t${b}`);
					})
					items.push(`${moment(line.timestamp).unix()} -   }`);
				} else {
					items.push(`${moment(line.timestamp).unix()} - ${mes}`);
				}
			})
		})
